...
def banner_when_attack(ip, port, threads):
    print(banner)
...
def check_target(target_ip, target_port, timeout=3):
    try:
        ...
        s.close()
        print(f"\n[{back_colors['green']}+{back_colors['reset']}] {styles['bright']}Target {target_ip}:{target_port} is reachable and open.{styles['reset']}")
        return True
    except socket.timeout:
        print(f"\n[{back_colors['yellow']}!{back_colors['reset']} {styles['bright']}Timeout: {target_ip}:{target_port} not responding.{styles['reset']}")
    except socket.error as e:
        print(f"\n[{back_colors['red']}!{back_colors['reset']}] {styles['bright']}Connection failed: {e}{styles['reset']}")
    return False
...
if check_target(target_ip, port_target):
    ...
    banner_when_attack(target_ip, port_target, num_thread)
